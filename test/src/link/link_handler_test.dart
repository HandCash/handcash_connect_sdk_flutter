...
class MockLinkListener extends Mock implements LinkListener {}
...
void main() {
  ...
  group('Correct links', () {
    test('Get token when url is correct in init app', () async {
      ...
      when(linkListener.getInitialUri()).thenAnswer((_) async => Uri.parse(correctUrl));
      when(linkListener.getUriLinksStream()).thenAnswer((_) => Stream.fromIterable([]));
      HandCashAuthTokenListener(linkListener: linkListener).listen(print);
      final String token = await HandCashAuthTokenListener(linkListener: linkListener).first;
    ...
    test('Get token when url is correct while app is open', () async {
      ...
      when(linkListener.getUriLinksStream()).thenAnswer((_) => Stream.fromIterable([Uri.parse(correctUrl)]));
      final String token = await HandCashAuthTokenListener(linkListener: linkListener).first;
    ...
    test('Get token when url is correct while app is open and link is opened before another links', () async {
      ...
      final Stream<String> token = HandCashAuthTokenListener(linkListener: linkListener);
  ...
  group('Incorrect Links', () {
    ...
    test('No event when url is incorrect in init app', () async {
      when(linkListener.getInitialUri()).thenAnswer((_) async => Uri.parse(incorrectUrl));
    ...
    test('No event when url is incorrect while app is open', () async {
      when(linkListener.getInitialUri()).thenAnswer((_) async => null);
      when(linkListener.getUriLinksStream()).thenAnswer((_) => Stream.fromIterable([Uri.parse(incorrectUrl)]));
      final Stream<String> token = HandCashAuthTokenListener(linkListener: linkListener);
