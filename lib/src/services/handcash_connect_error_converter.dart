...
class HandCashConnectErrorConverter extends ErrorConverter {
  ...
  FutureOr<Response> convertError<ResultType, ItemType>(Response response) =>
      throw _parseCloudApiException(response);
  ...
  CloudApiException _parseCloudApiException(Response response) {
    ...
    final String? message =
        data.containsKey('message') ? data['message'] as String : '';
    final int statusCode = response.statusCode;
    final Map<int, BuildException> exceptionsByCode = {
      400: (String message) => CloudApiException.invalidRequest(
          message: message, info: data['info'] as Map<String, dynamic>? ?? {}),
      401: (String message) =>
          CloudApiException.unauthenticated(message: message),
      403: (String message) =>
          CloudApiException.invalidAuthentication(message: message),
      409: (String message) => CloudApiException.conflict(message: message),
      413: (String message) =>
          CloudApiException.contentTooLarge(message: message),
      410: (String message) => CloudApiException.deprecated(message: message),
      450: (String message) =>
          CloudApiException.outdatedAppVersion(message: message),
      500: (String message) => CloudApiException.serverError(message: message),
      503: (String message) =>
          CloudApiException.externalServiceNotAvailable(message: message),
    };
